...
export function DataTableRowActions<TData>({
  ...
  const onConfirm = async () => {
    try {
      ...
      const result = await deleteBooking(row.getValue("id")).unwrap();
      if (result.success) {
        toast.success("Booking deleted successfully!", { id: "delete-Booking" });
      } else {
      ...
      const errorMessage =
        err?.message || "Failed to delete the Booking. Please try again.";
      toast.error(errorMessage, { id: "delete-Booking" });
    }
  };
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          <DropdownMenuItem>
            <Link
              href={`/dashboard/bookings/detail/${row.getValue("id")}`}
          ...
          <DropdownMenuItem>
            <Link
              href={`/dashboard/bookings/edit/${row.getValue("id")}`}
              className="flex w-full content-between gap-3"
            >
